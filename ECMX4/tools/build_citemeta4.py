...
OUT = "/mnt/user-data/outputs/ECMX4/tools/citemeta4.json"
...
def main():
    meta, ft = {}, set()
    for f in ("pool_rc.json", "pool_rc2.json", "pool_col.json", "pool_med.json"):
            ...
            for doi, r in it.items():
                ...
                for a in ("pmid", "pmcid"):
                    if r.get(a):
                        add(meta, r[a], rec)
    # 전문 확인 표시
    for idx in ("ft4_index.json", "ft3_index.json", "ft_index.json"):
    ...
    for k, r in meta.items():
        if r.get("pmcid") and str(r["pmcid"]).upper() in ft:
            r["ft"] = True
    # 특허
    for f in ("claims4.json", "claims3.json", "fpo_claims.json"):
        p = os.path.join(P, f)
